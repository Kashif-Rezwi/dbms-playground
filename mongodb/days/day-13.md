...
🟢 **P1.** Classify with the framework (5-line verdict each): (a) user's shipping address; (b) ALL of a user's orders; (c) a course's 10 lessons; (d) an author's 50,000 articles; (e) a post's first 3 comments + count.
🟢 **P2.** On `ecommerce`: fetch 3 orders, then their user names app-side (findOne per order — an N+1, felt). Then add `user_name` (hybrid) to one order copy and feel the read simplify. Write the drift chore that comes with it.
🟡 **P3.** Write-amp demo: `docs` with a 100-element array — `$push` 5 times; `refs` version (100 small docs + parent). Which update pattern moves more data and why?
🟡 **P4. ⭐ Predict first:** you embed the customer's full address in every order. The customer moves. How many documents must update — and the drift bug when the app forgets? Then the twist: for ORDER HISTORY, keeping the old address is *correct* (a snapshot!). Which semantics apply where? (SQL Day 26's history-vs-copy debate, document edition.)
🔴 **P5.** The ghost-document experiment: insert an order with `user_id: 999` — no error! Find it. Write the integrity check (collect used user_ids, diff against real ids). One-line lesson on referential integrity here.
🔴 **P6. From memory:** the 5-question framework + the hybrid's exact trade.
