# Day 15 — Many-to-Many + Modeling Patterns → 🏗️ Project 4

**Track:** MongoDB · **Stage:** 4 — Data Modeling · **Difficulty:** 🔴 · **Milestone:** Project 4
...
🟢 **P1.** Design followers for the social app — choose a pattern, defend with the four forces, sketch the query "who does user 7 follow?" and "who follows user 7?" (Both directions! Which pattern answers BOTH cheaply — that's the classic gotcha.)
🟢 **P2.** Design enrollments-with-grades (join collection + one hybrid snapshot field). Write the drift chore.
🟡 **P3.** Subset pattern, live: `products` gets `reviews_cache: { avg, count, top: [first 2] }` (in a copy of the collection) — query "product 9 with rating summary" in ONE read. Then update a review and… the cache lies. Write the two maintenance options (app-updates; scheduled recompute).
🟡 **P4. ⭐ Predict first:** which query gets cheap and which gets expensive with two-sided follow arrays: "A follows B?" vs "B's followers" (answer per pattern choice from P1). Then verify your design on the social dataset's real `followers` collection (it's a join-collection — compare!).
🔴 **P5.** Schema versioning, mini-drill: you have `users_v1` docs without `preferences`; design the v2 migration (read old docs → $set defaults → mark `schema_version: 2`) and run it on a copy. Two lines: why is in-place, lazy migration preferred over a big-bang rewrite?
🔴 **P6.** From memory: all seven pattern names + one line each.
