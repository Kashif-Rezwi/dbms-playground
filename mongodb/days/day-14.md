...
**Track:** MongoDB · **Stage:** 4 — Data Modeling · **Difficulty:** Intermediate
...
[Beginner] **P1.** Classify + shape (write the document sketch): (a) user → shipping address; (b) blog post → comments; (c) manufacturer → millions of cars; (d) course → 12 lessons; (e) conversation → years of messages.
[Beginner] **P2.** On `saas`: tasks reference projects (`project_id`) — the child-holds-parent pattern. Query "project 4's tasks" — then add the index that makes it instant (Day 16 preview: `db.tasks.createIndex({ project_id: 1 })`).
[Intermediate] **P3.** Feel the bad direction: create a `bad_user` doc with `order_ids: [1..12]` — fine now. Write the one-line reason it dies at 10k orders, and which direction scales instead.
[Intermediate] **P4. Predict first:** the shared dataset models orders→items how? (Check `ecommerce.orders` — *embedded* items!) Why is embedding right there, but user→orders referenced? (Bounded: orders max out at a few items; a user's lifetime orders are unbounded.) Say the boundary in numbers.
[Intermediate] **P5.** The 1:1 split decision: design `user_private` (ssn, tax details) separate from `users` — and write TWO reasons from today's fundamentals (size-class separation isn't one — what are the real ones? security partitioning + access patterns/read performance).
[Advanced] **P6.** Bucket-build: create `messages_bucketed` from the social `messages` data — group by conversation... our messages lack conversation ids — so bucket by *receiver pair*: `{ pair: [min_id, max_id], page: 1, messages: [...] }` for pairs (1,3) etc. Then query pair (1,3)'s messages — one doc read. Write when this beats one-message-per-doc.
[Advanced] **P7. From memory:** the three tiers + which side holds the reference in each.
