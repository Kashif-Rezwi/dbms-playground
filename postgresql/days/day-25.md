...
🟢 **P1.** Make corpses: `UPDATE big_users SET name = name WHERE id <= 50000;` → check `n_dead_tup`. `VACUUM big_users;` → re-check. Where did they go? (Space is *reusable*, not returned to the OS — check `pg_relation_size` before/after: same!)
🟢 **P2.** `\h VACUUM` — read the honest description of VACUUM vs VACUUM ANALYZE; one line each.
🟡 **P3.** Bloat generator + FULL: update a big chunk 3×, VACUUM (size unchanged), then VACUUM FULL (size drops!) — but from a second session, watch it block a concurrent SELECT. Two lessons, one experiment.
🟡 **P4.** Visibility map, verified: Day 14 P7's repeat — Index Only Scan with Heap Fetches, VACUUM, re-run → Heap Fetches ≈ 0. Write the two-line explanation.
🟡 **P5. ⭐ Predict first:** after updating 50k rows with NO vacuum, what does `SELECT count(*) FROM big_users;` read — and what does a "dead" count read? (MVCC: counts read *visible* versions — unchanged. Bloat is invisible to queries; it only costs time and space.)
🟡 **P6.** `SHOW autovacuum;` — generate churn on a small table; watch `last_autovacuum` update (patience, or force VACUUM to see the effect manually).
🔴 **P7.** Run the health-check query on all your databases; interpret each table's dead_pct in one line; name the table you'd investigate first and why.
🔴 **P8.** The "autovacuum isn't keeping up" diagnosis tree, written: three root causes (long transactions; threshold scaling on huge tables; anti-wraparound vacuum pressure) + the tell-tale + fix for each. A genuine on-call skill.
