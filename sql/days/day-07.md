...
🟢 **P1.** Find all unassigned tasks (`assignee_id IS NULL`). Predict the count first.
🟢 **P2.** Find all projects **with** a deadline. Then all projects, with missing deadlines shown as `2099-12-31` (COALESCE).
🟢 **P3.** List tasks with a column `is_late` = TRUE when `status <> 'done'` and `completed_at IS NULL` and... actually simpler: `completed_at IS NULL AND status <> 'todo'`. Name the flag `stuck`.
🟡 **P4.** Bucket invoices by status: `'overdue'` → `'🚨'`, `'open'` → `'⏳'`, else `'✔️'`. Sort by the bucket.
🟡 **P5. ⭐ Predict first:** exact output of both:
...
🟡 **P6.** For every task: title, plus `days_taken` = `completed_at - created_at`. Which completed tasks took more than 30 days? (Some rows will be NULL — why?)
🟡 **P7. From memory:** users whose `renews_at` is NULL in subscriptions.
🔴 **P8.** For each project (GROUP BY comes tomorrow — use plain SELECT with CASE today): show project tasks with a computed column `urgency` = 'high' priority AND not done → `'urgent'`; deadline passed (before today) → `'late'`; else `'ok'`. Use `CURRENT_DATE`.
