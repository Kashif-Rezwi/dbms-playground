...
🟢 **P1.** Create the `delivered_orders` view. SELECT from it. Then `\dv` (list views).
🟢 **P2.** Create `product_reviews` and find the top-rated products through it.
🟡 **P3.** Create `user_spending` (above — type it yourself). Find all users with total IS NULL — what does the NULL mean here? (Day 7 + Day 12 reunion!)
🟡 **P4.** Prove views are live: INSERT a new review via the *base* table, SELECT from `product_reviews` — is it there?
🟡 **P5.** Create materialized view `product_stats` (avg rating per product). Insert a new review into the base table. Is `product_stats` fresh? REFRESH it. Now?
🟡 **P6. ⭐ Predict first:** does this work — and why?
...
🔴 **P7. From memory:** create a view `inactive_users` (name, email of `is_active = FALSE` users) and query it for cities.
🔴 **P8.** Design decision: monthly revenue — view or materialized view? Argue in 3 lines: staleness tolerance, query cost, refresh cost.
