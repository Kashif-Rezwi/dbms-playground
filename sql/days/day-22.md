...
🟢 **P1.** Load `perf_lab` (`./scripts/utilities/load-large-postgres.sh`), turn on `\timing`, and run the `user_id = 417` query *before* indexing. Record the ms.
🟢 **P2.** Create the index. Rerun. Record the ms. Write one sentence: how many times faster?
🟢 **P3.** `\d big_orders` — find your index in the listing.
🟡 **P4.** Now time a query that filters on `ordered_at` (a *different* column): `SELECT count(*) FROM big_orders WHERE ordered_at = DATE '2025-01-15';` — slow again? Why didn't the user_id index help?
🟡 **P5.** Create an index on `ordered_at`, rerun P4's query. Compare.
🟡 **P6. ⭐ Predict first:** will this query use your `user_id` index? Why or why not? Then find out (Day 23 gives the tool, but you can guess by timing):

```sql
SELECT * FROM big_orders WHERE user_id = 417 OR status = 'pending';
```

🟡 **P7. From memory:** create an index on `big_order_items.order_id`. Time the difference on `SELECT * FROM big_order_items WHERE order_id = 100000;`
🔴 **P8.** Range query: time `SELECT * FROM big_orders WHERE ordered_at BETWEEN '2025-01-01' AND '2025-01-31'` before and after an index on `ordered_at`. Does an index help ranges, or only equality? Write one sentence.
🔴 **P9.** The write cost, measured: time `INSERT INTO big_users ... ` (one row, any values, id 60000) — then DROP both indexes on big tables that reference writes... actually simpler: create 3 more indexes on big_orders, then insert one row and time it. Now drop all 5 indexes, insert, time again. Feel the write tax.
