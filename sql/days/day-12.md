...
🟢 **P1.** Every user with their order count — **including users with zero orders**. (`COUNT(o.id)`, not `COUNT(*)` — why?)
🟢 **P2.** Every user with order ids; users without orders appear once. (This is P1's little sibling.)
🟢 **P3.** Users who have never written a review. LEFT JOIN pattern.
🟡 **P4.** Products never ordered — with their category_id and price.
🟡 **P5.** Categories that have **no products** in them. (Careful: which side is categories?)
...
🟡 **P7. From memory:** reviews with product names (every review, even if... there's a subtlety — which join type and why?).
🔴 **P8.** Orders with **no payment** (pending or cancelled but unpaid — but be careful: one delivered order also has no payment? Check with data). Show order id + status.
🔴 **P9.** Users whose **only** orders are cancelled (they ordered, but nothing ever shipped/delivered). Hard! Hint: LEFT JOIN with condition `status <> 'cancelled'`, then `IS NULL`... on what?
