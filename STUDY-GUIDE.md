...
4. Practice             — do ALL exercises, in order, including predict-the-result
...
## Beyond the Daily Lesson

The daily files are the spine; these are the reinforcement layers — use them at stage boundaries, not instead of the days:

- **Drill packs** (`sql/exercises/`) — predict-the-result and write-from-memory sets for extra repetition between stages.
- **Interview banks** (`shared/interview-questions/`) — beginner/intermediate/advanced questions with follow-ups, plus `mini-interview-checks.md`: short out-loud simulations timed to each stage boundary (the track roadmaps point to the right check at the right day).
- **Cheat sheets** (each track's `cheat-sheet.md`, `shared/cheat-sheets/`) — for *revision after practice*, never during from-memory tasks.
- **GLOSSARY.md** — when a term feels fuzzy, read its two lines, then find the day that teaches it.
